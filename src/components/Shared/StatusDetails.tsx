...
const StatusDetails = () => {
  ...
  const totalMinutes = todPlan.reduce(
    (sum: number, workout: IWorkoutDataType) => sum + (workout.duration || 0),
    0,
  );

  const totalCalories = todPlan.reduce(
    (sum: number, workout: IWorkoutDataType) =>
